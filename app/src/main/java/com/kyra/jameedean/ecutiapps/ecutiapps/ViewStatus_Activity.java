...
public class ViewStatus_Activity extends AppCompatActivity{
    ...
    private void actionNotification(DatabaseError error, int successResourceId) {
        // close activity
        if(error == null) {
            Toast.makeText(ViewStatus_Activity.this, successResourceId, Toast.LENGTH_SHORT).show();
            finish();
        } else {
            Toast.makeText(ViewStatus_Activity.this, error.getCode(), Toast.LENGTH_SHORT).show();
        }
    }
